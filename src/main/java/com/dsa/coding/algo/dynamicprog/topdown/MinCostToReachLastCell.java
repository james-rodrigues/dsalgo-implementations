...
public class MinCostToReachLastCell {
	...
	public int findMinCost_aux(int[][] dp, int[][] array, int row, int col) {
		...
		if (dp[row][col] == 0) {// If we have not solved this problem previously, only then solve it
			int minCost1 = findMinCost_aux(dp, array, row - 1, col);// Case#1: Get min cost if we go 'up' from current
																	// cell
			int minCost2 = findMinCost_aux(dp, array, row, col - 1);// Case#2: Get min cost if we go 'left' from current
																	// cell
