...
public class LongestPalindromicSubstring {
	...
	private int lps_aux(String string, int startIndex, int endIndex) {
		...
		if (startIndex == endIndex) {// BASE CASE - If both the index are at same position then its a palidrome as
										// its 1 character.
			return 1;
		}
		int c1 = 0;
		if (string.charAt(startIndex) == string.charAt(endIndex)) { // CASE#1 - If index pointed characters matches then
																	// we
